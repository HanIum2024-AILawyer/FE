...
function Joinpage2() {
    ...
    if (email.trim() === "") {
      ...
      try {
        const response = await fetch(
          "http://15.164.102.12:8080/api/v1/verification/email",
          {
            method: "POST",
            headers: {
  ...
  const handleVerificationCodeCheck = async () => {
    ...
    if (verificationCodeInput.trim() === "") {
      ...
      try {
        const response = await fetch(
          "http://15.164.102.12:8080/api/v1/verification/code",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleNicknameCheck = async () => {
    ...
    try {
      const response = await fetch(
        `http://15.164.102.12:8080/api/v1/users/nickname/duplication?nickname=${encodeURIComponent(
          nickname
        )}`,
        {
          method: "GET",
  ...
  const handleSignup = async () => {
    ...
    try {
      const response = await fetch(
        "http://15.164.102.12:8080/api/v1/users/signup",
        {
          method: "POST",
